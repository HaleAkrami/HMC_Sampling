...
def continuous_energy_from_image_numpy(coord, xp, yp, zp, fill_value=None):
    """
...
def interp2d_numpy(x,y,xp,yp,zp,fill_value):
    ...
    ix = torch.clip(torch.searchsorted(xp, x, right=True), 1, len(xp) - 1)
    iy = torch.clip(torch.searchsorted(yp, y, right=True), 1, len(yp) - 1)
    ...
    if fill_value is not None:
        oob = torch.logical_or(
            x < xp[0], torch.logical_or(x > xp[-1], torch.logical_or(y < yp[0], y > yp[-1]))
        )
        z = torch.where(oob, torch.tensor(fill_value, dtype=z.dtype), z)
